...
class Api::V1::DashboardAndHistoryControllerTest < ActionDispatch::IntegrationTest
  setup do
    ...
    @task = TaskTemplate.create!(title: "알고리즘", points: 15, target_count: 2, position: 0, kind: "algorithm")
    @other_task = TaskTemplate.create!(title: "포트폴리오", points: 20, target_count: 1, position: 1, kind: "portfolio")
  ...
  test "dashboard returns task progress, daily summary, and the active focus session" do
    ...
    PointEvent.create!(daily_task_completion: completion, source_device: @device, activity_date: @date, event_type: "task_completion", points: 15, idempotency_key: "dashboard-event", occurred_at: completed_at)
  ...
  test "history fills the requested range and includes reversed point events" do
    ...
    PointEvent.create!(source_device: @device, activity_date: @date, event_type: "adjustment", points: 5, idempotency_key: "history-event", occurred_at: occurred_at, reversed_at: occurred_at + 1.hour)
